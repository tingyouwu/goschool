 ...
 * 
 */
public class PublishDALEx extends SqliteBaseDALEx {
	...
	public static PublishDALEx get() {
		return SqliteDao.getDao(PublishDALEx.class);
	}
	
	/**
	 * 功能描述：保存用户信息至数据库，有相同的e号即更新，没有就做插入操作
	 */
	public void save(PublishDALEx user){
		AppDBHelper db = null;
		try {
			db = getDB();
	 ...
	 * 通过获取e号获取用户
	 */
	public PublishDALEx getUserByUsernumber(String usernumber) {
		PublishDALEx user = null;
		Cursor cursor = null;
		try {
